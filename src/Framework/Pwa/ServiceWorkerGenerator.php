...
const ALL_CACHES = [CACHE_NAME, CACHE_ASSETS, CACHE_DYNAMIC];
    ...
    event.respondWith(
        fetch(request)
            .then((response) => {
                ...
                caches.open(CACHE_DYNAMIC).then((cache) => cache.put(request, responseToCache));
                return response;
            })
            .catch(() => {
            ...
            caches.match(request)
                .then((cachedResponse) => {
                    ...
                    return fetch(request).then((response) => {
                        const responseToCache = response.clone();
                        caches.open(CACHE_ASSETS).then((cache) => cache.put(request, responseToCache));
                        return response;
                    });
                })
        );
